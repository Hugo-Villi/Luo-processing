...
if side_choice==1
   ...
   for i=1:(size(markers_values,1)-1)                                  %-1 otherwise it will crash trying to reach the last frame + 1
    ...
    %each file
    [Reve,Leve,peaks_num] = get_event(I_luo_mod,sweep,PeakHeight,PeakDistance,PeakProeminence,strike_off_choice,X_RTOE,X_LTOE);
else
    ...
    %detection and discretization)
    for i=3:3:size(markers_values,2)
        displacement_x_r(:,i/3)=displacement(:,i-2);
        displacement_y_r(:,i/3)=displacement(:,i-1);
        displacement_z_r(:,i/3)=displacement(:,i);
        displacement_x_l(:,i/3)=displacement(:,i-2);
        displacement_y_l(:,i/3)=displacement(:,i-1);
        displacement_z_l(:,i/3)=displacement(:,i);
    end
    %%displacement histogram
    ...
    discretization_z_l=linspace(min_z_l,max_z_l_l,n);
    
    discretization_global=linspace(minimum,maximum,n);
    
    Cx_luo_r=compute_C_luo_mod_disc(n,displacement_x_r,discretization_x_r,labels,sweep);  %returns the C matrix
    Cy_luo_r=compute_C_luo_mod_disc(n,displacement_y_r,discretization_y_r,labels,sweep);
    Cz_luo_r=compute_C_luo_mod_disc(n,displacement_z_r,discretization_z_r,labels,sweep);
    %C_max_min_luo=compute_C_luo_mod_disc(n,displacement,discretization_global,labels,sweep);
    
    Cx_luo_l=compute_C_luo_mod_disc(n,displacement_x_l,discretization_x_l,labels,sweep);  %returns the C matrix
    Cy_luo_l=compute_C_luo_mod_disc(n,displacement_y_l,discretization_y_l,labels,sweep);
    Cz_luo_l=compute_C_luo_mod_disc(n,displacement_z_l,discretization_z_l,labels,sweep);
    %C_max_min_luo=compute_C_luo_mod_disc(n,displacement,discretization_global,labels,sweep);
    ...
    %events, for each dimension. get_error3_with_plot returns the plot for
    %each file
    [Reve,Leve_r,peaks_num_r] = get_event(I_luo_mod_r,sweep,PeakHeight,PeakDistance,PeakProeminence,strike_off_choice,X_RTOE,X_LTOE);
    [Reve_l,Leve,peaks_num_l] = get_event(I_luo_mod_l,sweep,PeakHeight,PeakDistance,PeakProeminence,strike_off_choice,X_RTOE,X_LTOE);
end
end